    ...
    S: Sink,
{
    ...
    fn fill(&mut self) -> Result<bool, S::Error> {
        ...
        // нам больше не нужен, так как искать больше нечего. Поэтому
...
impl<'s, M: Matcher, S: Sink> MultiLine<'s, M, S> {
    ...
    pub(crate) fn run(mut self) -> Result<(), S::Error> {
        if self.core.begin()? {
            ...
            if !self.core.detect_binary(self.slice, &binary_range)? {
                ...
                // Позаботимся об оставшемся контексте после последнего
                // совпадения.
                if keepgoing {
    ...
    fn sink(&mut self) -> Result<bool, S::Error> {
        ...
        // Мы откладываем передачу совпадения, чтобы убедиться, что мы
        // группируем соседние совпадения вместе в одном sink. Соседние
        // совпадения — это различные совпадения, которые начинаются и
        // заканчиваются на одной и той же строке соответственно. Это
        // гарантирует, что одна строка никогда не передаётся более одного
        // раза.
        match self.last_match.take() {
            ...
            Some(last_match) => {
                // Если строки в предыдущем совпадении перекрываются со
                // строками в этом совпадении, то просто увеличиваем
                // совпадение и продолжаем. Это происходит, когда следующее
                // совпадение начинается на той же строке, на которой
                // заканчивается последнее совпадение.
                //
                // Обратите внимание, что технически мы не требуем строгого
                // перекрытия здесь. Вместо этого мы требуем только, чтобы
                // строки были соседними. Это предоставляет большие блоки
                // строк принтеру и приводит к лучшему поведению в целом
                // в отношении того, как обрабатываются замены.
                //
                // См.: https://github.com/BurntSushi/ripgrep/issues/1311
                // А также связанный коммит, исправляющий #1311.
    ...
    fn sink_matched(&mut self, range: &Range) -> Result<bool, S::Error> {
        if range.is_empty() {
            // Единственный способ получить пустую строку для совпадения —
            // это если мы сопоставим позицию сразу после последнего байта,
            // который мы ищем, и где этот последний байт также является
            // завершителем строки. Мы никогда не хотим сообщать об этом
            // совпадении, и мы знаем, что на этом закончили, поэтому
            // останавливаем поиск.
            return Ok(false);
        }
    ...
    /// Продвинуть позицию поиска на основе предыдущего совпадения.
    ///
    /// Если предыдущее совпадение имеет нулевую ширину, то это продвигает
    /// позицию поиска на один байт за конец совпадения.
