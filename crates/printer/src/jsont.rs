// Этот модуль определяет типы, которые мы используем для сериализации JSON. Мы специально
// опускаем десериализацию, отчасти потому, что в настоящее время для неё нет явного
// случая использования, но также потому, что десериализация усложнит вещи.
// А именно, типы ниже спроектированы таким образом, что допускают сериализацию JSON
// с минимальным выделением памяти или без него. Выделение памяти часто бывает
// удобным для десериализации, поэтому эти типы стали бы немного сложнее.
...
/// Data представляет вещи, которые выглядят как строки, но на самом деле могут не быть
/// валидным UTF-8. Для обработки этого `Data` сериализуется как объект с одним
/// из двух ключей: `text` (для валидного UTF-8) или `bytes` (для невалидного UTF-8).
///
/// Счастливый путь — валидный UTF-8, который передаётся как есть, поскольку
/// он нативно поддерживается JSON. Когда обнаруживается невалидный UTF-8, тогда он
/// представляется как произвольные байты и кодируется в base64.
...
impl<'a> Data<'a> {
    ...
    #[cfg(not(unix))]
    fn from_path(path: &Path) -> Data<'_> {
        // Использование потерианного преобразования означает, что некоторые пути не будут
        // точно воспроизведены, но неясно, что нам следует делать. Serde отвергает
        // пути, не являющиеся UTF-8, а OsStr сериализуются как последовательность UTF-16
        // кодовых единиц на Windows. Ни один из вариантов не кажется подходящим для этого случая использования,
        // поэтому мы делаем простое дело на данный момент.
...
/// Реализует "стандартное" кодирование base64, как описано в RFC 3548[1].
///
/// Мы реализуем своё собственное здесь вместо того, чтобы привлекать что-то более тяжёлое, как
/// крейт `base64`. В частности, нам действительно не важна производительность здесь,
/// поскольку это используется только для данных или путей к файлам, которые не являются
/// валидным UTF-8.
///
...
fn base64_standard(bytes: &[u8]) -> String {
    ...
    match it.remainder() {
        ...
        _ => unreachable!("остаток должен иметь длину < 3"),
    }
    out
}
