/// Ошибка, возникающая при разборе описания размера, читаемого человеком.
///
/// Эта ошибка предоставляет удобное для конечного пользователя сообщение,
/// описывающее, почему описание не удалось разобрать и каков ожидаемый формат.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self.kind {
            ...
            InvalidInt(ref err) => write!(
                f,
                "неверное целое число найдено в размере '{}': {}",
                self.original, err
            ),
            Overflow => write!(f, "слишком большой размер в '{}'", self.original),
        }
    }
}
